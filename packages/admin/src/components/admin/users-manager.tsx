...
export function UsersManager({ initial }: { initial: User[] }) {
  ...
  return (
    <div className="p-6 space-y-6">
        ...
        <div className="flex gap-1">
          {(["all", ...ROLES] as const).map((r) => (
            <button key={r} onClick={() => setRoleFilter(r as Role | "all")}
              className={`rounded-md px-3 py-1.5 text-xs font-medium capitalize transition-colors ${roleFilter === r ? "bg-neutral-900 text-white" : "border border-neutral-200 text-neutral-600 hover:bg-neutral-50"}`}>
              {r}
            </button>
          ))}
        </div>
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="rounded-lg border border-neutral-200 bg-white p-6 space-y-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-lg rounded-xl bg-white shadow-xl">
            <div className="flex items-center justify-between border-b border-neutral-200 px-6 py-4">
              <h2 className="font-semibold text-neutral-900">Edit user</h2>
        ...
        {visible.length === 0 ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-neutral-100">
              {visible.map((user) => (
                <tr key={user.id} className={`hover:bg-neutral-50 ${user.suspended ? "opacity-60" : ""}`}>
                  ...
                  <td className="px-4 py-3">
                    {user.suspended ? (
                      <span className="inline-flex rounded-full px-2 py-0.5 text-xs font-medium bg-red-100 text-red-700">Suspended</span>
                    ) : (
                      <span className="inline-flex rounded-full px-2 py-0.5 text-xs font-medium bg-green-100 text-green-700">Active</span>
                    )}
                  </td>
                  <td className="px-4 py-3 text-neutral-500">{new Date(user.createdAt).toLocaleDateString()}</td>
